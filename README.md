...
- 数据库备份与 `app-files` 对象存储备份必须分别做恢复演练。
- 数据策略：注册满四年自动删除账号并清理个人信息，保留去个人化业务记录；管理员保留独立的毕业处置入口，可提前处置；截图和活动材料按 180 天规则处理。治理页会明确显示预览、执行和失败状态，不能据此推断生产清理已启用。
- 管理员不得共用账号；每名管理员使用独立账号，关键操作写入 `audit_logs`。
