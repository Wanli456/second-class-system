...
export default function Home() {
  ...
  if (user) {
    ...
    const certificationEntries: PortalEntry[] = [
      ...
      { href: '/submit/scoring', label: '赋分材料', detail: '提交活动赋分材料或班级赋分表', icon: Award, tone: 'bg-amber-50 text-amber-700', show: hasPermission(user, 'canSubmitScoring') || hasPermission(user, 'canImportScoring') },
      { href: '/other-college-registration', label: '其他学院登记', detail: '登记其他学院校级活动的赋分材料', icon: Award, tone: 'bg-amber-50 text-amber-700', show: hasPermission(user, 'canRegisterOtherCollege') },
    ...
    const departmentEntries: PortalEntry[] = [
      ...
      { href: '/leave-slip/records', label: '假条查看与对比', detail: '查看上传假条、核对原假条', icon: ClipboardList, tone: 'bg-slate-100 text-slate-700', show: hasPermission(user, 'canQueryLeave') || hasPermission(user, 'canManageOriginalLeave') },
      { href: '/leave-slip/originals/submit', label: '提交原假条', detail: '提交需要归档的原始假条', icon: FileCheck, tone: 'bg-slate-100 text-slate-700', show: hasPermission(user, 'canSubmitOriginalLeave') },
      { href: '/attendance-work', label: '考勤工作安排', detail: '按周提交和查对考勤工作安排', icon: ClipboardList, tone: 'bg-teal-50 text-teal-700', show: hasPermission(user, 'canManageAttendanceWork') },
      { href: '/evening-study', label: '晚自习查询', detail: '查看晚自习请假与考勤安排', icon: Moon, tone: 'bg-indigo-50 text-indigo-700', show: hasPermission(user, 'canViewEveningStudy') },
    ].filter((entry) => entry.show);
    ...
    const quickHrefs = Array.from(new Set([
      ...
      ...(hasPermission(user, 'canManageAttendanceWork') ? ['/attendance-work'] : []),
      ...(hasPermission(user, 'canReviewLeave') ? ['/leave-slip/review'] : []),
      ...((hasPermission(user, 'canQueryLeave') || hasPermission(user, 'canManageOriginalLeave')) ? ['/leave-slip/records'] : []),
      ...(hasPermission(user, 'canSubmitOriginalLeave') ? ['/leave-slip/originals/submit'] : []),
      ...(hasPermission(user, 'canViewEveningStudy') ? ['/evening-study'] : []),
      ...(hasPermission(user, 'canSubmitActivity') ? ['/submit'] : []),
      ...(hasPermission(user, 'canViewSubmissionStatus') ? ['/submit/status'] : []),
      ...((hasPermission(user, 'canSubmitScoring') || hasPermission(user, 'canImportScoring')) ? ['/submit/scoring'] : []),
      ...(hasPermission(user, 'canRegisterOtherCollege') ? ['/other-college-registration'] : []),
