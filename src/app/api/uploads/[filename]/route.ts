...
export async function GET(request: NextRequest, { params }: { params: Promise<{ filename: string }> }) {
  ...
  if (!isAdmin) {
    ...
    const reference = await queryOne<{ allowed: boolean }>(
      `SELECT EXISTS (
        SELECT 1 FROM activities WHERE $1 IN (plan_file_url, record_file_url, activity_image_url)
        ...
        UNION ALL SELECT 1 FROM activity_submissions WHERE $1 IN (plan_file_url, record_file_url, activity_image_url)
          AND ($2 = activity_submitter_id OR leader_ids LIKE '%"' || $2 || '"%' OR $3)
