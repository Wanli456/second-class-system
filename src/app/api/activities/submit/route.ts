...
export async function POST(request: NextRequest) {
  try {
    ...
    const repeated = await queryOne<Record<string, unknown>>('SELECT * FROM activity_submissions WHERE idempotency_key=$1', [idempotencyKey]);
    if (repeated) {
