...
function FilePicker({ label, file, existingUrl, existingName, onChange, imageOnly = false }: { label: string; file: File | null; existingUrl: string | null; existingName: string | null; onChange: (file: File | null) => void; imageOnly?: boolean }) {
  ...
  return <div className="min-w-0 text-sm font-medium">{label}<label className={'mt-1 flex cursor-pointer items-center gap-2 rounded-md border border-dashed px-3 py-3 focus-within:ring-2 focus-within:ring-teal-600 text-sm font-normal ' + (file ? 'border-emerald-300 bg-emerald-50 text-emerald-700' : 'text-gray-500')}><Upload className="h-4 w-4 shrink-0" /><span className="truncate">{file?.name || existingName || (existingUrl ? '已上传文件，选择新文件可替换' : '选择文件')}</span><input type="file" aria-label={label.replace(' *', '')} className="sr-only" accept={imageOnly ? '.jpg,.jpeg,.png,.gif,.webp,.bmp' : '.pdf,.doc,.docx,.jpg,.jpeg,.png'} onChange={(e) => {
    ...
    e.target.value = '';
    if (!selected) return;
    if (imageOnly && (!/\.(jpe?g|png|gif|webp|bmp)$/i.test(selected.name) || !selected.type.startsWith('image/'))) { setFileError('请选择 JPG、PNG、GIF、WebP 或 BMP 图片'); return; }
    if (selected.size > 5 * 1024 * 1024) { setFileError('文件大小不能超过5MB'); return; }
    setFileError(''); onChange(selected);
  }} /></label>{imageOnly && <p className="mt-1 text-xs font-normal text-gray-500">必传 1 张，不超过 5MB；支持 JPG、PNG、GIF、WebP、BMP，点击缩略图放大。</p>}{fileError && <p role="alert" className="mt-1 text-xs text-red-600">{fileError}</p>}{isImage && preview ? <ImageUploadPreviews imageUrls={[preview]} altPrefix={label.replace(' *', '')} onRemove={() => { onChange(null); setFileError(''); }} /> : imageOnly && existingUrl ? <ImageUploadPreviews imageUrls={[existingUrl]} altPrefix="已提交活动图片" /> : null}</div>;
}
