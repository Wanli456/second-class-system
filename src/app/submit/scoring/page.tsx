...
export default function SubmitScoringPage() {
  ...
  const canImportClassScoring = hasPermission(user, 'canImportScoring');

  // 只有班级赋分表提交权限的用户：只显示导入窗口，不显示赋分材料查询
  if (!canSubmitMaterials && canImportClassScoring) {
    return (
      <DashboardLayout title="班级赋分表提交" user={user}>
        <div className="space-y-6"><ClassScoringImport /></div>
      </DashboardLayout>
    );
  }
