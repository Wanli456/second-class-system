...
export function ImageUploadPreviews({ imageUrls, fileNames, altPrefix, onRemove }: ImageUploadPreviewsProps) {
  const [selectedIndex, setSelectedIndex] = useState<number | null>(null);
  const selectedUrl = selectedIndex === null ? null : imageUrls[selectedIndex] || null;
  const selectedFileName = selectedIndex === null ? null : fileNames?.[selectedIndex] || null;
  ...
  return (
    <>
      ...
      <FilePreviewDialog open={selectedIndex !== null} onOpenChange={(open) => { if (!open) setSelectedIndex(null); }} url={selectedUrl} fileName={selectedFileName || (selectedIndex === null ? null : altPrefix + "-" + (selectedIndex + 1) + ".jpg")} title={altPrefix + "预览"} />
    </>
  );
}
