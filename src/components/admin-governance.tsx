...
type AuditItem = { id: string; action: string; resourceType: string; resourceId: string | null; details: unknown; createdAt: string };
...
export default function AdminGovernance({ users, onUsersRefresh }: { users: GovernanceUser[]; onUsersRefresh: () => Promise<void> }) {
  ...
  return <div className="space-y-5">
    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6"><h2 className="font-semibold text-slate-950">操作日志</h2><p className="mt-1 text-sm text-slate-500">管理员可查看治理与业务操作记录。</p><div className="mt-4 overflow-x-auto"><table className="w-full min-w-[42rem] text-left text-sm"><thead className="border-b border-slate-200 text-xs text-slate-500"><tr><th className="px-2 py-2">时间</th><th className="px-2 py-2">动作</th><th className="px-2 py-2">资源</th><th className="px-2 py-2">详情</th></tr></thead><tbody className="divide-y divide-slate-100">{audits.map(item => <tr key={item.id}><td className="px-2 py-2 text-slate-500">{formatDate(item.createdAt)}</td><td className="px-2 py-2 font-medium text-slate-800">{item.action}</td><td className="px-2 py-2 text-slate-600">{item.resourceType}{item.resourceId ? ` · ${item.resourceId}` : ''}</td><td className="max-w-xs truncate px-2 py-2 text-slate-500">{typeof item.details === 'string' ? item.details : JSON.stringify(item.details || {})}</td></tr>)}</tbody></table>{audits.length === 0 && <p className="py-6 text-center text-sm text-slate-500">暂无日志。</p>}</div><Pager page={auditPage} total={auditTotal} pageSize={PAGE_SIZE} onChange={(page) => void run(() => loadAudits(page), '日志列表已更新')} /></section>
  </div>;
}
